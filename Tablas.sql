...
IF EXISTS (SELECT 1 FROM sys.databases WHERE name = 'SUPERMERCADO_Grupo7')
BEGIN
    PRINT 'La base de datos SUPERMERCADO_Grupo7 ya existe';
END
ELSE
BEGIN
	CREATE DATABASE SUPERMERCADO_Grupo7;
    PRINT 'La base de datos SUPERMERCADO_Grupo7 creada.';
END;
go
...
----Creacion de los esquemas con verificacion de si ya existe
...
drop table esquema_operaciones.detalledeventa
go
drop table esquema_operaciones.notadecredito
go
drop table esquema_producto.lineadeproducto
go
drop table esquema_producto.producto
go
drop table esquema_ventas.ventasregistradas
go
drop table esquema_operaciones.factura
go
drop table esquema_operaciones.mediosdepago
go
drop table esquema_persona.cliente
go
drop table esquema_persona.empleado
go
drop table esquema_sucursal.sucursales
go
...
--------CREACION DE LA TABLA EMPLEADO CON INCRIPTACION DE LOS DATOS IMPORTANTES
...
------------TABLAS DE MEDIOS DE PAGO, FACTURA  Y NOTA DE CREDITO
...
---------------TABLA DE VENTAS REGISTRADAS Y DETALLE DE VENTA
